...
const siteCss = `<style>
        ...
        #images>li{width:22em;height:5em}
        #images>li>a{align-items:center}
        #images>li>a>img{height:5em;max-width:16em}
    </style>`;
...
const footerScript = `<script>function goBack(){const e=document.URL;window.open(e.substring(0, e.lastIndexOf('/')),'_blank')}</script>`;
...
function makeHtml(paths: string[]): Promise<string> {
    ...
    return readdir(pathString, {withFileTypes: true}).then(contents => {
        ...
        const returnString = `<!DOCTYPE html><html lang="en"><head>` + headerMeta + `<title>` + title + `</title>` 
            + siteCss + headerScript + `</head><body><h2>` + header + ` | <a onclick="goBack()" class="back">< back</a></h2>
