...
const uploadHtml = '<form id="uploadForm" action="/fileupload" method="post" enctype="multipart/form-data"><input type="file" multiple="multiple" name="filetoupload"><input type="submit" value="Upload"></form>'
...
const siteCss = `<style>
        ...
        li{display:inline-flex;margin-right:1%;width:16em;justify-content:start;align-items:center}
        li>a:hover{background-color:firebrick}
        .back{color:orangered;cursor:pointer}
        li>a>svg,li>a>img{height:1em;margin-right:.5em}
        li>a{color:white;text-decoration:none;display:flex;padding:5% 4% 5% 1%}
        #images>li{width:22em;height:5em}#images>li>a{align-items:center}
        #images>li>a>img{height:5em;max-width:16em;border:1px solid white}</style>`;
    ...
    return new Promise((resolve, reject) => {
        ...
        file.on('data', (chunk: Buffer) => {
            ...
            if(data.startsWith('-----------------------------')) {
                ...
                writeStream = createWriteStream(join(pathForFile, fileName));
